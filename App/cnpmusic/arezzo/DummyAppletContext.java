...
public class DummyAppletContext
	...
	return (Iterator<String>)(new Object());
    }
    ...
    public DummyAppletContext( String args[] ) {
        super ( "Arezzo" );
        try {
            Applet applet = (Applet)new Arezzo() ;

            init( applet, 640, 480, args, 0 );
        }
        catch ( Exception e ) {
            e.printStackTrace();
            System.exit( 1 );
...
/************ AppletStub methods *************/
    ...
    public URL getDocumentBase() {
	    ...
	    path = path.substring( 0, path.length()-"dummy.html".length()-1 );
...
/************ AppletContext methods *************/
	...
	if( target.equals( "Arezzo MIDI" )) {
		...
		status.setText( "Unable to play MIDI file " + url +
				" in target: " + target );
	    }
	}
	else if( target.equals( "Manuel" )) {
		 ...
		 tp.setPage( new URL( getCodeBase()+"/../manual.html" ));
	     } 
	     catch (Exception e) {
		 e.printStackTrace();
	     }
	}
	else if( target.equals( "Arezzo Study" )) {
	    try {
